...
{
   wiringPiI2CWriteReg8(_tsl2561CircuitID, reg, value);
   //wiringPiI2CWrite(_tsl2561CircuitID, reg);
   //wiringPiI2CWrite(_tsl2561CircuitID, value);
}
...
{
   //wiringPiI2CWrite(_tsl2561CircuitID, reg);
   //return wiringPiI2CRead(_tsl2561CircuitID);
   return wiringPiI2CReadReg8(_tsl2561CircuitID, reg);
}
...
uint16_t TSL2561::read16(uint8_t reg)
{
  ...
  wiringPiI2CWrite(_tsl2561CircuitID, reg);
  delay(14);
  ::read(_tsl2561CircuitID, datas, 2);
...
{
  _tsl2561CircuitID = wiringPiI2CSetup(_addr);
...
uint32_t TSL2561::calculateLux(uint16_t broadband, uint16_t ir)
{
  ...
  {
      printf("%u > %u || %u > %u\n\n", broadband, clipThreshold, ir, clipThreshold);     
    return 0;
  }
...
void TSL2561::getEvent(sensors_event_t *event)
{
  ...
  event->timestamp = 0;//event->timestamp = millis();
