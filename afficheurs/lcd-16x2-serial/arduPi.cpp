...
SerialPi::SerialPi(const char *serialPortUSB){
	 REV = getBoardRev();
    serialPort = NULL;
    strcpy(this->serialPortUSB, serialPortUSB);
    timeOut = 1000;
}
...
void SerialPi::begin(int serialSpeed){
   ...
   {
      if ((sd = open(serialPortUSB, O_RDWR | O_NOCTTY | O_NDELAY | O_NONBLOCK)) == -1){
         fprintf(stderr,"Unable to open the serial port %s - \n", serialPortUSB);
         exit(-1);
      }
   }
