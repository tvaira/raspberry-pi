    ...
    Adafruit_GFX(ST7735_TFTWIDTH, ST7735_TFTHEIGHT) 
{
   ...
   hwSPI = true;
   _sid  = _sclk = 0;
   //printf("<Adafruit_ST7735> CS = %d\n", _cs);
   //printf("<Adafruit_ST7735> DC = %d\n", _rs);
}

Adafruit_ST7735::~Adafruit_ST7735()
{
   fillScreen(ST7735_WHITE);
   writecommand(ST7735_SWRESET); //software reset, puts display into sleep
   delay(200); //wait 200mS for controller register init
   writecommand(ST7735_SLPOUT); //sleep out.
   delay(200); //wait 200mS for TFT driver circuits
   writecommand(ST7735_DISPOFF); //display off!
   delay(200);
   SPI.end();
   //printf("<~Adafruit_ST7735>\n");
}
...
inline void Adafruit_ST7735::spiwrite16(uint16_t c) 
{
   uint8_t hi = c >> 8, lo = c;
